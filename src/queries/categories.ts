...
export class CategoryQueries {
    ...
    async listCategories(includeDeleted = false): Promise<PartCategory[]> {
        const query = includeDeleted ? aql`FOR cat IN part_cats FILTER cat._deleted == null RETURN cat` : aql`FOR cat IN part_cats RETURN cat`;
    ...
    async getCategoryTree(parentKey?: string, maxDepth = 5): Promise<CategoryTree> {
        const buildTree = (catKey: string, level: number): any => {
            ...
            return {
                ...
                code: `CODE-${catKey}`,
                depth: level,
                children: [], // Simplified - in real implementation, would fetch children
            };
        };
        ...
        if (!parentKey) {
            ...
            return {
                ...
                code: 'ROOT',
                depth: 0,
                children: roots.map((r) => buildTree(r._key, 1)) as CategoryTree[],
            };
        }

        return buildTree(parentKey, 1);
    }
    ...
    async cascadeDelete(categoryKey: string): Promise<{
        ...
        const descended = await this.db.query(aql`
      FOR doc IN 1..100 OUTBOUND ${categoryId} part_cats_links
        RETURN doc._key
    `);
        const descendants = await descended.all();

        // Delete all descendant edges
        for (const descendantKey of descendants) {
            await this.db.query(aql`
        FOR edge IN part_cats_links
          FILTER edge._to == ${'part_cats/' + descendantKey}
          REMOVE edge IN part_cats_links
      `);

            await this.db.query(aql`
        FOR edge IN part_cats_links
          FILTER edge._from == ${'part_cats/' + descendantKey}
          REMOVE edge IN part_cats_links
      `);
        }
    ...
    `);
        ...
        return {
            ...
            deletedEdgeCount: descendants.length * 2 + 2,
        };
    }
